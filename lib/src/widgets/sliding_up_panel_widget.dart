...
const Duration _kSlidingUpPanelDuration = Duration(milliseconds: 800);
...
class _SlidingUpPanelWidgetState extends State<SlidingUpPanelWidget> with SingleTickerProviderStateMixin<SlidingUpPanelWidget> {
  ...
  void _handleDragEnd(DragEndDetails details) {
    if (details.velocity.pixelsPerSecond.dy < -_kMinFlingVelocity) {
      ...
      collapse();
    } else if ((_animationController.value >= (anchorFraction + upperBound) / 2)) {
      expand();
    } else if (_animationController.value >= _kCloseProgressThreshold && _animationController.value < (anchorFraction + upperBound) / 2) {
      anchor();
    } else {
      collapse();
    }
  ...
  ///Expand the panel
  void expand() {
    _animationController?.animateTo(1.0, curve: Curves.linearToEaseOut, duration: _kSlidingUpPanelDuration);
  ...
  ///Hide the panel
  void hide() {
    _animationController?.animateTo(0.0, curve: Curves.linearToEaseOut, duration: _kSlidingUpPanelDuration);
